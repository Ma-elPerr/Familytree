...
export function matchDNA(dnaMatches: DNAMatch[], graph: GenealogyGraph): MatchResult[] {
  ...
  // Pre-compute normalized names for all individuals to avoid O(N*M) normalizeName calls
  ...
  for (const match of dnaMatches) {
    ...
    for (const { node, normGraphName } of normalizedIndividuals) {
      ...
      // Short-circuit: Levenshtein distance cannot be less than the absolute difference in string lengths
