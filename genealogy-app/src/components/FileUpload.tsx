    ...
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors">
          ...
          <p className="text-sm text-gray-600 mb-2">Arquivo GEDCOM (.ged)</p>
          <input
            type="file"
            accept=".ged"
          ...
          />
          {gedcomFile && <p className="mt-2 text-xs text-green-600">Arquivo selecionado: {gedcomFile.name}</p>}
        </div>

        {/* CSV Upload */}
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors">
          <UploadCloud className="mx-auto h-12 w-12 text-gray-400 mb-3" />
          <p className="text-sm text-gray-600 mb-2">Matches de DNA (.csv)</p>
          <input
            type="file"
            accept=".csv"
            ...
            className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
          />
          {csvFile && <p className="mt-2 text-xs text-green-600">Arquivo selecionado: {csvFile.name}</p>}
        </div>
      </div>
